...
from lua_bridge import apply_instinct
...
async def main():
    ...
    # =====================================================
    # FUNGSI 4: POST-PROCESSOR RESMI (LUA INSTINCT)
    # Backend terakhir sebelum response ke user
    # =====================================================
    async def post_process_response(msg):
        try:
            data = json.loads(msg.data.decode())
            ...
            mode = data.get("_meta_mode", "direct_response")
            final_response = apply_instinct(mode, response_text)
            ...
            # Fallback aman: kirim response asli
